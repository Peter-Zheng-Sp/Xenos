...
INT_PTR MainDlg::OnLoadProfile( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    std::wstring path;
#ifdef USE64
    if (OpenSaveDialog( L"USBaby profiles (*.upr64)\0*.upr64\0", 1, path ))
#else
    if (OpenSaveDialog( L"USBaby profiles (*.upr)\0*.upr\0", 1, path ))
#endif
    {
        // Reset loaded images
        _profileMgr.config().images.clear();
...
INT_PTR MainDlg::OnSaveProfile( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    std::wstring path;
#ifdef USE64
    if (OpenSaveDialog( L"USBaby profiles (*.upr64)\0*.upr64\0", 1, path, true, L"upr64" ))
#else
    if (OpenSaveDialog( L"USBaby profiles (*.upr)\0*.upr\0", 1, path, true, L"upr" ))
#endif
    {
        SaveConfig( path );
        _status.SetText( 0, blackbone::Utils::StripPath( path ) );
